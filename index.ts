...
let messageCounter = new Gauge({name: "http_messages", help: "Messages/second"});
...
setInterval(() => {
    ...
    for (let x in connectionPools) {
        ...
        if (now - connectionPoolOpen[x] > 43200000) // close after 12h
            for (let y of connectionPools[x])
                y.close();
    }
}, 10 * 60 * 1000);

setInterval(() => {
    messageCounter.set(messageCount);
    messageCount = 0;    
}, 1000);
